...
| [아키텍처](./architecture.md) | AST 중심 데이터 흐름, 상태 관리, 화면 구성, 모듈 책임 | 전체 구조를 처음 파악할 때 |
...
| [테스트와 유지보수](./testing.md) | 자동 테스트, 수동 점검표, 결합된 상수, 알려진 제한 | 변경을 검증하거나 배포를 준비할 때 |
| [구현 단계 기록](./milestones/README.md) | 1~6단계 목표, 구현 내용, 산출물과 완료 기준 | 구현 순서와 변경 배경을 추적할 때 |
...
- 디자인 토큰과 레이아웃: [`src/index.css`](../src/index.css)
