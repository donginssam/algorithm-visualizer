...
- 조건 분기 → 반복이면 합류점과 모든 입출력 연결선을 삭제하고 나머지는 보존한다
- `아니면` 본문이 없는 조건도 바꿀 수 있다
- 반복 → 조건 분기이면 본문이 합류점에서 만난다
- 바꿨다 되돌려도 기호를 잃지 않고, 연결이 끊긴 화살표도 남지 않는다
- 합류점에 입력만·출력만·양방향 연결이 있는 경우도 모두 삭제한다
- 아직 잇지 않은 짝 합류 기호는 반복으로 바꾸면 지우고, 조건 분기로 바꾸면 그대로 써서 잇는다
- 조건→반복→조건 왕복에서 아니면 본문을 합류점 뒤로 밀어내지 않고, 짝 합류점만 두 갈래 아래에 준비한다
- 미완성 그래프를 반복으로 바꿀 때는 옮길 연결이 없으면 종류만 바꾼다
- 연결 전 조건→반복→조건 전환에서도 합류점을 다시 생성하며 중복 생성하지 않는다
- 예 본문만 연결된 판단은 본문 아래에 합류점을 만들고 기존 연결을 보존한다
- 바깥 반복 본문의 끝에서 조건→반복 전환은 합류점의 복귀선도 삭제하고, 반복→조건 전환은 복귀선 성격을 유지한다

### `removeNodes.test.ts`

기호를 지울 때 판단 기호의 짝 합류 기호가 함께 지워지는지 검사합니다.

- 조건 분기 판단을 지우면 짝 합류 기호와 닿아 있던 화살표를 함께 지우고, 갈래 본문은 남긴다
- 짝 id가 아닌 합류점이라도 두 갈래가 만나는 곳이면 함께 지운다
- 반복 판단을 지워도 아직 잇지 않은 짝 합류 기호는 함께 지운다
- 다른 판단의 합류점, 판단이 아닌 기호, 합류 기호만 지우는 경우, 없는 id는 건드리지 않는다
- 이름이 바뀐 합류점의 노드·화살표 동시 삭제와 입력 불변성
- 다음 판단으로 직접 합류하거나 외부 입력을 받는 합류점은 보존
- 중첩 분기와 삭제 순서가 다른 다중 삭제, 화살표 단독 삭제
- 삭제 작업을 한 단계로 undo/redo할 때 노드와 화살표 전체 복원

### `graphTopology.test.ts`

학생이 캔버스에서 화살표를 이을 때의 판단 규칙을 검사합니다.

- 복귀선 판정: 본문에서 돌아오는 화살표는 복귀선, 바깥에서 들어오는 첫 화살표는 아님
- 복귀선을 먼저 그려도 뒤에 잇는 바깥 화살표를 복귀선으로 보지 않음(예전 버그)
- 본문이 아직 판단과 이어지지 않았으면 이미 들어온 화살표로 가림, 조건 판단은 복귀선 없음
- 연결 허용: 자기 자신·없는 기호·끝에서 나감·시작으로 들어옴은 거부
- 한 연결점에서 화살표 하나(판단은 예·아니오를 따로 셈), 연결점 없는 화살표는 '다음'으로 셈
- 들어오는 화살표는 하나, 합류점과 반복 판단만 둘까지
...
- AST는 글이 문법에 맞으면 글에서 다시 만들고, 문법 오류일 때만 저장된 AST를 모양 확인 후 사용
...
| 판단 기호 삭제   | `removeNodes.ts`, 팔레트·자동 배치의 합류 기호 id 규칙, `FlowCanvas`의 세 삭제 경로                                                       |
